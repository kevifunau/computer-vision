...
def Labelcheck(image,x,y):
    ...
    nw ,n = 0,0
    if y>0:
        nw = isVaild(image,x,y-1)
    if x>0:
        n = isVaild(image,x-1,y)

    # nw and n have no labels
    if nw ==False and n ==False:
        LABEL +=1
        CONNECT[LABEL] = LABEL
        return LABEL
    # nw and n have label
    elif nw !=False and n != False:

        CONNECT[nw] = min(Connectbase(nw),Connectbase(n))
        CONNECT[n] =CONNECT[nw]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 5 and sys.argv[1] == "--input" and sys.argv[3] == "--size" and sys.argv[4].isdigit():
        ...
        size = int(sys.argv[4])
        output_image = None
    elif len(sys.argv) == 7 and sys.argv[1] == "--input" and sys.argv[3] == "--size" and sys.argv[4].isdigit() and sys.argv[5] == '--optional_output':
        input_image = sys.argv[2]
        size = int(sys.argv[4])
